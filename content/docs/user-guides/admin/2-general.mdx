...
-   Endkunden können ihre hinterlegten Stammdaten einsehen. Als Nächstes ist geplant, dass Endkunden ihre Stammdaten künftig auch selbst ändern können und dass diese Änderungen anschließend automatisch im System des White-Label-Kunden aktualisiert werden.
- Nutzer können sich über eine eindeutige Kunden-Identifikation (z. B. Vertragsnummer, Kundennummer, GP-Nummer) mit weiteren Kunden verknüpfen. Dadurch erhalten sie automatisch Zugriff auf alle zugehörigen Objekte und Zähler – eine manuelle Freigabe entfällt.
...
Individuelle Support-Kontakte werden unter „Kunden" im Admin-Portal verwaltet. Durch Auswahl eines Kunden können dessen Kontaktdaten eingesehen und bearbeitet werden. Zusätzlich lässt sich die Funktion „Support-Kontakt" auf der Einstellungsseite über den neuen Button „Konfigurieren" aktivieren und einrichten.
